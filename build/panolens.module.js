  ...
  Object.create(EventDispatcher.prototype),
  {
    ...
    addControlBar: function () {
      ...
      bar.toggle = function () {
        ...
        const styleTranslate = bar.isHidden
          ? "translateY(0)"
          : "translateY(-100%)";
    ...
    setupGoogleMapAPI: function (apiKey) {
      ...
      if (script.readyState) {
        ...
        script.onreadystatechange = function () {
          if (
            script.readyState === EVENTS.LOADED ||
...
StereoImagePanorama.prototype = Object.assign(
  Object.create(ImagePanorama.prototype),
  {
...
StereoVideoPanorama.prototype = Object.assign(
  Object.create(VideoPanorama.prototype),
  {
...
PanoMomentPanorama.prototype = Object.assign(
  Object.create(PanoMoment.prototype),
  {
    ...
    updateHeading: function () {
      if (!this.momentData) return;
      const {
        momentData: { max_horizontal_fov },
      } = this;
      this.material.uniforms.offset.value.x =
        (max_horizontal_fov / 360 + 0.25) % 1;
      // control update
      this.resetControlLimits(false);
    ...
    resetAzimuthAngleLimits: function (reset = false) {
      ...
      const delta = Math$1.degToRad(
        (0.95 * min_vertical_fov - camera.fov) / 2
      );
      const angles = {
      ...
      Object.assign(
        OrbitControls,
        reset ? { minPolarAngle, maxPolarAngle } : angles
      );
    },
    /**
     * Calculate FOV limit
    ...
    calculateFOV: function (fov, horizontal) {
      ...
      return (
        ((2 * Math.atan(Math.tan((fov * Math.PI) / 360) * factor)) / Math.PI) *
        180
      );
    },
    /**
    ...
    resetFOVLimits: function (reset = false) {
      ...
      if (fovH > momentData.min_horizontal_fov * 0.95) {
        camera.fov = this.calculateFOV(
          momentData.min_horizontal_fov * 0.95,
          false
        );
...
function PanoMomentRegular(identifier) {
  PanoMoment.call(this, identifier);
  this.scale2D = new Vector2(1, 1);
}
PanoMomentRegular.prototype = Object.assign(
  Object.create(PanoMoment.prototype),
  {
    ...
    createMaterial: function (
      ...
      const uniforms = UniformsUtils.clone(BackgroundShader.uniforms);
    ...
    update2DGeometryScale: function (reset = false) {
      ...
      const {
        momentData: { aspect_ratio },
      } = this;
      const { fov, aspect } = this.camera;
      const scale =
        2 * Math.tan((fov * Math.PI) / 360) * Math.min(aspect_ratio, aspect);
...
function OrbitControls(object, domElement) {
  ...
  this.rotateLeft = function (angle) {
    ...
    angle =
      this.momentum && !this.autoRotate
        ? (angle /= this.momentumFactor)
        : angle;
    thetaDelta -= angle;
  };
  ...
  this.update = function (ignoreUpdate) {
    ...
    if (this.enableDamping === true && this.speedLimit !== Number.MAX_VALUE) {
      thetaDelta = Math$1.clamp(
        thetaDelta,
        -this.speedLimit,
        this.speedLimit
      );
      phiDelta = Math$1.clamp(phiDelta, -this.speedLimit, this.speedLimit);
    }
    ...
    if (
      ...
      this.enableDamping === true &&
      ((this.momentum &&
        (state === STATE.ROTATE || state === STATE.TOUCH_ROTATE)) ||
        state === STATE.NONE)
    ) {
  ...
  function onKeyDown(event) {
    if (
      ...
      return;
    const updatedMomentumKeydownFactor =
      scope.momentum && !scope.autoRotate
        ? scope.momentumKeydownFactor * scope.momentumFactor
        : scope.momentumKeydownFactor; // Handle difference in necessary rotateSpeed constants.
    switch (event.keyCode) {
      case scope.keys.UP:
...
function DeviceOrientationControls(object) {
  ...
  this.update = function ({ theta = 0 } = { theta: 0 }) {
    ...
    if (device) {
      ...
      const beta = device.beta ? Math$1.degToRad(device.beta) : 0; // X'
      const gamma = device.gamma ? Math$1.degToRad(device.gamma) : 0; // Y''
  ...
  this.getBeta = function () {
    const { deviceOrientation: device } = scope;
    return device && device.beta ? Math$1.degToRad(device.beta) : 0;
  };
}
...
function CardboardEffect(renderer) {
  ...
  const geometry = new PlaneBufferGeometry(1, 1, 10, 20)
  ...
  this.render = function (scene, camera, panorama) {
    const stereoEnabled =
      panorama instanceof StereoImagePanorama ||
      panorama instanceof StereoVideoPanorama;
...
function Viewer(options = {}) {
  ...
  if (enableReticle) this.enableReticleControl();
  else this.registerMouseAndTouchEvents();
...
Viewer.prototype = Object.assign(
  Object.create(EventDispatcher.prototype),
  {
    ...
    setupCamera: function (
      cameraFov,
      ratio,
      camera = new PerspectiveCamera(cameraFov, ratio, 1, 10000)
    ) {
    ...
    setupTween: function () {
      this.tweenCanvasOpacityOut
        .to({}, 500)
        .easing(Tween.Easing.Exponential.Out);
      this.tweenCanvasOpacityIn
        .to({}, 500)
        .easing(Tween.Easing.Exponential.Out);
      ...
      if (ep instanceof Panorama && lp !== ep) {
        ...
        if (lp) {
          ...
          if (lp._onEnterFadeStart) {
            lp.removeEventListener(
              EVENTS.ENTER_FADE_START,
              lp._onEnterFadeStart
            );
    ...
    enableControl: function (index = CONTROLS.ORBIT) {
      const {
        control: { index: currentControlIndex },
        OrbitControls,
      ...
      if (index === currentControlIndex) {
        // ignore
        return;
      } else if (index === CONTROLS.DEVICEORIENTATION) {
        // device orientation
    ...
    onWindowResize: function (windowWidth, windowHeight) {
      let width, height;
      const expand =
        this.container.classList.contains(EVENTS.CONTAINER) ||
    ...
    outputPosition: function () {
      ...
      if (intersects.length > 0) {
        ...
        point.sub(world);
        const message = `"_x": ${point.x.toFixed(2)}, "_y": ${point.y.toFixed(
          2
        )}, "_z": ${point.z.toFixed(2)}`;
        if (point.length() === 0) {
          return;
        }
    ...
    onPanoramaDispose: function (panorama) {
      const { scene } = this;
      const infospotDisposeMapper = (infospot) =>
        infospot.toPanorama !== panorama ? infospot : infospot.dispose();
      ...
      scene.traverse((object) => {
        if (object instanceof Panorama) {
          object.linkedSpots = object.linkedSpots
            .map(infospotDisposeMapper)
            .filter((infospot) => !!infospot);
        }
      });
...
export { BasicPanorama, CONTROLS, CameraPanorama, CubePanorama, CubeTextureLoader, DataImage, EVENTS, EmptyPanorama, GoogleStreetviewPanorama, ImageLittlePlanet, ImageLoader, ImagePanorama, Infospot, LittlePlanet, MODES, Media, PanoMoment, PanoMomentPanorama, PanoMomentRegular, Panorama, REVISION, Reticle, STEREOFORMAT, Stereo, StereoImagePanorama, StereoVideoPanorama, THREE_REVISION, THREE_VERSION, TextureLoader, VERSION, VideoPanorama, Viewer, Widget };
